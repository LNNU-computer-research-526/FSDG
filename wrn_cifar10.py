...
parser.add_argument('--dataset', type=str, default='cifar10', choices=['MNIST', 'cifar10', 'cifar100','AR','FashionMNIST','usps'])
...
if  opt.dataset != 'MNIST':
    ...
    if opt.dataset == 'cifar10':
        ...
        #net = Alexnet.AlexnetHalf().cuda()
        net = wideresnet.Wide_ResNet40_1().cuda()
        net = nn.DataParallel(net)
...
for epoch in range(opt.n_epochs):
    ...
    avg_loss = 0.0
    if opt.dataset != 'MNIST':
        adjust_learning_rate(optimizer_Ds, epoch, opt.lr_Ds)

    for i in range(120):

        # 优化 Gt
        z1 = Variable(torch.randn(opt.batch_size, opt.latent_dim),
                      requires_grad=True).cuda()  # 生成一批与真实数据集batch_size一样128，潜在空间维度一样的随机样本1000
        # print(len(z1))
        optimizer_Gt1.zero_grad()
        # print(z1)
        img_t1 = generator_t(z1)  # Xt
        # outputs_T, features_T= teacher(img_t1, out_feature=True)
        outputs_T, features_T = teacher(img_t1)
        ...
        # loss_kd = inter_class_relation(net(img_s.detach()), teacher(img_s.detach()) + intra_class_relation(net(img_s.detach()), teacher(img_s.detach())))
        # loss_kd = kdloss(net(img_s.detach()), teacher(img_s.detach())) + dist_criterion(net(img_s.detach()), teacher(img_s.detach())) + angle_criterion(net(img_s.detach()), teacher(img_s.detach()))
        # loss_kd = kdloss(net(img_s.detach()),teacher(img_s.detach()))+ inter_class_relation(net(img_s.detach()), teacher(img_s.detach()) + intra_class_relation(net(img_s.detach()), teacher(img_s.detach())))
        #loss_kd = inter_class_relation(net(img_s1.detach()), teacher(img_s1.detach())) + intra_class_relation(net(img_s1.detach()), teacher(img_s1.detach()))
        loss_kd = inter_class_relation(net(img_s1.detach())[0], teacher(img_s1.detach())[0]) + intra_class_relation(
             net(img_s1.detach())[0], teacher(img_s1.detach())[0])
        # loss_kd = kdloss(net(img_s.detach()),teacher(img_s.detach()))
        loss_kd.backward()
        optimizer_Ds.step()
        ...
        loss_relation = dist_criterion(net(img_s2)[0], teacher(img_s2)[0]) + angle_criterion(net(img_s2)[0],teacher(img_s2)[0])
        ...
        if i == 1:
            print("[Epoch %d/%d]  [loss_T: %f] [loss_S: %f] [loss_KD: %f]  " % (epoch, opt.n_epochs, loss_T2.item(),loss_S.item(),loss_kd.item()))
